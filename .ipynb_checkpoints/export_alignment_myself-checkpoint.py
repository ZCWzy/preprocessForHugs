# # python export_alignment.py --scene_dir E:\download\hugsDataset\seattle\sparse  --images_dir E:\download\hugsDataset\seattle\images --raw_smpl E:\download\hugsDataset\seattle\smpl_pred --smpl_estimator="romp"


# # python export_alignment_myself.py --scene_dir E:\download\hugsDataset\seattle\sparse  --images_dir E:\download\hugsDataset\seattle\images --romp E:\download\hugsDataset\seattle\smpl_pred --hmr=E:\download\hugsDataset\seattle\4d_humans\demo_.pkl
...
#         return read_vibe_estimates(opt.romp)
#     elif opt.smpl_estimator == 'romp':
#         assert os.path.isdir(opt.romp)
#         dump_path = os.path.abspath(os.path.join(opt.romp, '../smpl_output_romp.pkl'))
#         dump_romp_estimates(opt.romp, dump_path, scene)
...
# def make_smpl_opt(hmr, scales, transl, rotations):
#     hmr_result = joblib.load(hmr)
#     body_pose = []
#     global_orient = []
#     bbox = []
#     betas = np.zeros((1, 10))
...
#         order='video'
#     )
#     romp = read_smpl(opt, scene)

#     assert len(romp['pose']) == len(scene.captures)
...
#         pts_3d = romp['joints3d'][i]
#         pts_2d = romp['joints2d_img_coord'][i]
...
#     make_smpl_opt(opt.hmr, scales, transls, rotations)
...
#     parser.add_argument('--romp', type=str, default=None, required=True)
#     parser.add_argument('--hmr', type=str, default=None, required=True)
#     parser.add_argument('--smpl_estimator', type=str, default="romp")
...
def read_4d_humans(images_dir,raw_smpl):
    img = Image.open(images_dir+"/00000.png") #读取第一张图片
